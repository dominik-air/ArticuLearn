...
describe("Badge Component", () => {
  ...
  it("renders correctly with all props", () => {
    ...
    expect(
      screen.getByText(`Congratulations, ${userName}!`),
    ).toBeInTheDocument();
    expect(
      screen.getByText(`You've achieved: ${achievement}`),
    ).toBeInTheDocument();
    expect(screen.getByRole("img", { name: "Achievement" })).toHaveAttribute(
      "src",
      imageUrl,
    );
  ...
  it("renders correctly without an image URL", () => {
    ...
    expect(
      screen.queryByRole("img", { name: "Achievement" }),
  ...
  it("includes the Share button with a Star icon", () => {
    ...
    expect(shareButton.closest("button")).toContainElement(
      screen.getByTestId("StarIcon"),
    );
  });
});
